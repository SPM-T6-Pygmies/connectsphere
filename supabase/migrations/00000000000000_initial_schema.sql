-- Baseline migration: this repo's tables predate the migrations folder and
-- were applied by hand rather than through the CLI. Timestamped 0 so it
-- always runs first, ahead of every real migration.
--
-- Content below is supabase/schema.sql, with the `registration` table
-- restored to the pre-attendee-identity shape the first three migrations
-- actually assume: schema.sql was hand-kept in sync with those migrations'
-- table changes as they landed, so replaying it verbatim ahead of them
-- collides on constraints they (re-)add. See the comments in that table for
-- specifics. Everything else here is unmodified.
...
create table registration (
  ...
  event_id                bigint not null references event (event_id) on delete cascade,
  session_id              bigint references session (session_id) on delete cascade,
  attendee_user_account_id bigint not null references user_account (user_account_id) on delete cascade,
...
-- The attendee identity columns (registration_reference, attendee_name,
-- attendee_email), their nullability changes and registration_attendee_identity_chk
-- are added by migrations/20260907132238_attendee_registration_identity.sql,
-- not here -- this baseline mirrors the team schema those migrations were
-- written against.
...
-- registration_one_live_per_attendee_email_uidx (the same rule keyed by the
-- lower-cased email, for the account-less attendee) is added by
-- migrations/20260907132238_attendee_registration_identity.sql, once
-- attendee_email exists.
